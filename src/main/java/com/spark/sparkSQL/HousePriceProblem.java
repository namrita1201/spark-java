...
public class HousePriceProblem {
  ...
  public static final String PRICE_SQ_FT = "Price SQ Ft";
  ...
  public static void main(String[] args) {
    ...
    Dataset<Row> dataset = dataFrameReader.option("header", true).csv("src/main/resources/RealEstate.csv");

    final Dataset<Row> rowDatasetCasted = dataset.withColumn(PRICE_SQ_FT, col(PRICE_SQ_FT).cast("long"))
        .withColumn(PRICE, col(PRICE).cast("long"));

    RelationalGroupedDataset groupedDatasetByLocation = rowDatasetCasted.groupBy("Location");
    groupedDatasetByLocation.agg(avg("Price SQ Ft"), max("Price")).orderBy(col("avg(" + PRICE_SQ_FT + ")").desc())
        .show();
